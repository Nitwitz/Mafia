...
namespace Mafia
{
    ...
    public partial class FormMenu : Form
    {
        ...
        public byte[] c_answer = new byte[1024];
        public StringBuilder c_str = new StringBuilder();
        public int c_bytes;
        ...
        private void _clientReaction()
        {
            ...
            do
            {
                ...
                if (c_message == "hui")
                {
                    byte[] c_data = Encoding.Default.GetBytes("message");
                    _client._socket.Send(c_data);
                    ListOfPlayers.Items.Add(c_message);
                    pnlGame.Refresh();
                ...
                {
                    Thread.Sleep(1000);
                    byte[] c_data = Encoding.Default.GetBytes("message_new");
                    _client._socket.Send(c_data);
                }
                //if (c_message.IndexOf("=").Equals(0))
